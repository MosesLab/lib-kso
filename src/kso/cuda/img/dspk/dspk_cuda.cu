...
np::ndarray kso::img::dspk::locate_noise_3D(const np::ndarray & cube, float std_dev, uint k_sz, uint Niter){
	...
	CHECK(cudaMemcpy(dev_d, dev, sz * sizeof(float), cudaMemcpyHostToDevice));
	...
	for(uint iter = 0; iter < Niter; iter++){
		...
		kso::img::dspk::calc_dev<<<blocks, threads>>>(dt_d, gm_d, dev_d, sz3, k_sz);
		CHECK(cudaDeviceSynchronize());


		// -----------------------------------------------------------------------
		// NEIGHBORHOOD STANDARD DEVIATION CONVOLUTION
		kso::img::dspk::calc_goodmap<<<blocks, threads>>>(std_dev, gm_d, dev_d, sz3, k_sz, newBad_d);
		CHECK(cudaDeviceSynchronize());
	...
	CHECK(cudaMemcpy(dev, dev_d, sz * sizeof(float), cudaMemcpyDeviceToHost));
...
__global__ void kso::img::dspk::calc_dev(float * dt, float * gm, float * dev, dim3 sz, uint k_sz){
