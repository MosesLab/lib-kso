...
__global__ void calc_gm(float * gm, uint * new_bad, float * dt, float * q2,
		float * t0, float * t1, dim3 sz, dim3 hsz) {
...
__global__ void calc_gm(float * gm, uint * new_bad, float * dt, float * q2,
		float * t0, float * t1, dim3 sz, dim3 hsz, uint nmet) {
...
__global__ void calc_thresh(float * t0, float * t1, float * hist, float * cs,
		dim3 hsz, float T0, float T1) {
...
__global__ void calc_hist(float * hist, float * dt, float * q2, float * gm,
		dim3 sz, dim3 hsz) {
	...
	if (gm[L] == 0.0)
		return;	// don't do anything if the goodmap marks this pixel as bad
...
void calc_quartile(float * q, float * dt, float * gm, float * tmp, dim3 sz,
		dim3 ksz, uint quartile) {
...
void calc_quartiles(float * q1, float * q2, float * q3, float * dt, float * gm,
		float * tmp, dim3 sz, dim3 ksz) {
	...
	for (uint quartile = 1; quartile <= 3; quartile++) {
		...
		calc_sep_quartile<<<blocks, threads>>>(Q, dt, gm, sz, ksz, xhat,
				quartile);
		calc_sep_quartile<<<blocks, threads>>>(tmp, Q, gm, sz, ksz, yhat,
				quartile);
		calc_sep_quartile<<<blocks, threads>>>(Q, tmp, gm, sz, ksz, zhat,
				quartile);
...
__global__ void calc_sep_quartile(float * q_out, float * q_in, float * gm,
		dim3 sz, dim3 ksz, dim3 axis, uint quartile) {
	...
	for (int i = -((int) kr1); i <= (int) kr1; i++) {
		...
		uint A = n.z * (z + a.z * i) + n.y * (y + a.y * i)
				+ n.x * (x + a.x * i);
		float u = q[A];
		...
		for (int j = -((int) kr1); j <= (int) kr1; j++) {
			...
			uint B = n.z * (z + a.z * j) + n.y * (y + a.y * j)
					+ n.x * (x + a.x * j);
			float v = q[B];
...
__global__ void calc_tot_quartile(float * q, float * dt, float * gm, dim3 sz,
		dim3 ksz, uint quartile) {
